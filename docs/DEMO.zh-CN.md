...
**界面发生什么：** 问题会标在正文和编辑器边缘；悬浮可查看原因与 Quick Fix，点击“问题”面板条目会聚焦精确范围。修复只修改该范围。
...
**界面发生什么：** WriteMelo 比较快照和当前正文，按词标出新增与删除并统计变更。恢复前必须确认，恢复后可以撤销。
...
**你做什么：** 打开“AI”，选择“关闭”“仅发送问题”或“问题 + 当前全文”。

**界面发生什么：** 弹窗明确区分三种状态：关闭时零 API 调用、零 AI 供应商费用；“仅发送问题”只发送问题；“问题 + 当前全文”会在每次提问时附带活动文档。

**你得到什么：** 没有账号、没有 AI 时，本地工具仍然完整可用；确实需要深入分析时才主动调用 AI。

**联网 / AI：** 打开设置和启用按需模式本身都不发送请求。只有用户真正提问才调用供应商，并可能消耗额度或产生费用；全文模式通常消耗更多 token。
...
WriteMelo 在不开启 AI 时提供一组边界明确的本地工具：有限补全减少输入，确定性诊断解释已覆盖的错误，快速修复精确修改，大纲检查结构，版本 Diff 让旧文字可检查。AI 是可选能力，不会静默接管草稿。
...
| 大幅修改后想找回旧草稿 | 展示词级 Diff 和变更统计 | 预览并确认恢复，需要时撤销 | 替换草稿前先看清改了什么 |
| 需要编辑本地文本或 Markdown | 打开并保存 `.txt`、`.text`、`.md`、`.markdown` | 用户选择来源和目标 | 不必复制粘贴即可处理真实文件 |
| 已有团队或学科词表 | 在本地导入 `.txt` 或 Hunspell `.dic` | 用户检查并选择文件 | 减少专有词反复误报 |
| 同时处理多封邮件或多篇文章 | 左栏保存并切换多个本地文档 | 用户新建、切换或删除 | 避免草稿散落在多个临时文件 |
| 不想注册账号、联网或使用 AI | 补全、拼写、诊断、大纲和历史都在本地工作 | AI 保持关闭 | 免费、离线也能获得核心价值 |
| 本地规则无法判断语气是否强硬 | 用户明确选择模式后再向 AI 提问 | 只有“问题 + 当前全文”会带上文档 | 只把 AI 用在规则工具不擅长的地方 |
...
下面列出的能力在设备上运行；不同写作任务所需深度不同，因此不再给出没有测量依据的百分比。AI 只处理确定性规则难以可靠覆盖的语义问题。
...
- 自动保存、多文档管理、词级版本 Diff、恢复确认和撤销。
- 打开与保存 `.txt`、`.text`、`.md`、`.markdown`。
- 从 `.txt` 和 Hunspell `.dic` 导入个人词汇。
- 中英文界面。
...
- 按学习等级调整候选。
- 根据成熟的本地使用记录模型调整候选排序（规划中；当前记录尚未形成完整学习闭环）。
...
WriteMelo 不宣称完整语法检查或通用术语一致性分析。当前实际提供：49,568 词条离线拼写词表、61 个排序补全词、7 个固定片段、文档实体复用，以及一小组经过测试的确定性写作规则。确实需要深层理解时，再由用户决定是否发送文本和承担供应商费用。
