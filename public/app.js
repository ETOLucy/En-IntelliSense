...
const HELP_I18N = {
  en: { api_key_help: 'Your API key stays on this computer and is protected by Windows encryption. It is never uploaded to En-IntelliSense servers.' },
  zh: { api_key_help: 'API Key 仅保存在这台电脑上，并由 Windows 加密保护；不会上传到 En-IntelliSense 服务器。' },
  es: { api_key_help: 'Tu clave API permanece en este equipo y está protegida por el cifrado de Windows. Nunca se carga en los servidores de En-IntelliSense.' },
  ja: { api_key_help: 'API キーはこの PC にのみ保存され、Windows の暗号化で保護されます。En-IntelliSense のサーバーには送信されません。' },
  ko: { api_key_help: 'API 키는 이 컴퓨터에만 저장되며 Windows 암호화로 보호됩니다. En-IntelliSense 서버로 업로드되지 않습니다.' },
  fr: { api_key_help: "Votre clé API reste sur cet ordinateur et est protégée par le chiffrement Windows. Elle n'est jamais envoyée aux serveurs En-IntelliSense." },
  de: { api_key_help: 'Ihr API-Schlüssel bleibt auf diesem Computer und wird durch die Windows-Verschlüsselung geschützt. Er wird nie auf En-IntelliSense-Server hochgeladen.' },
  pt: { api_key_help: 'Sua chave de API fica neste computador e é protegida pela criptografia do Windows. Ela nunca é enviada aos servidores do En-IntelliSense.' },
  ar: { api_key_help: 'يبقى مفتاح API على هذا الكمبيوتر وتحميه آلية تشفير Windows. ولا يُرفع إلى خوادم En-IntelliSense.' },
  hi: { api_key_help: 'आपकी API key इसी कंप्यूटर पर रहती है और Windows एन्क्रिप्शन से सुरक्षित होती है। इसे En-IntelliSense सर्वर पर अपलोड नहीं किया जाता।' },
  ru: { api_key_help: 'Ключ API хранится только на этом компьютере и защищён шифрованием Windows. Он не отправляется на серверы En-IntelliSense.' }
};
